...
class LightOpenID
{
    ...
    function __construct($site_url)
    {
        ...
        var_dump($this->trustRoot);    //    'http://localhost'

        $uri = rtrim(preg_replace('#((?<=\?)|&)openid\.[^&]+#', '', $_SERVER['REQUEST_URI']), '?');  //   '/eslip-plugin/openid/example.php'

        $this->returnUrl = $this->trustRoot . $uri;  // RETURN URL, LA DEBEMOS PONER NOSOTROS 'http://localhost/eslip-plugin/openid/example.php'

        var_dump($this->returnUrl);

        $this->data = ($_SERVER['REQUEST_METHOD'] === 'POST') ? $_POST : $_GET;

        var_dump($this->data); //vacio
    }

    function prep_trust_url($site_url)
    {
        $parse_url = parse_url($site_url);

        if (! isset($parse_url['host']))
        {
            $url = explode('/', $parse_url['path'])[0];
        }
        else
        {
            $url = $parse_url['host'];
    ...
    protected function request_curl($url, $method='GET', $params=array(), $update_claimed_id)
    {
        ...
        if($method == 'HEAD' && curl_getinfo($curl, CURLINFO_HTTP_CODE) == 405) {   //El servidor no tiene HEAD entonces pido GET con la opcion de que devuelva las cabeceras
     ...
     * from them
     */
    protected function getValueOfHtmlTag($content, $tag, $attrName, $attrValue, $valueName)
    {
    ...
    function discover($url)
    {
        ...
        for ($i = 0; $i < 5; $i ++) {
            if ($yadis) {   // http://en.wikipedia.org/wiki/Yadis           
